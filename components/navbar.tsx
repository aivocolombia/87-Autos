...
export default function Navbar() {
  ...
  const getCurrentLogo = () => {
    if (pathname === "/") {
      // Dynamic logo based on slide (only on home page)
      if (currentSlide === 1) {
  ...
  const getTextColor = () => {
    if (pathname === "/") {
      // Dynamic colors based on slide (only on home page)
      // Slides 0 and 1: white text
      // Slide 2 and beyond: black text
      return (currentSlide <= 1) ? "text-white drop-shadow-lg" : "text-gray-900 drop-shadow-sm"
    }
  ...
  const getHoverColor = () => {
    if (pathname === "/") {
      return (currentSlide <= 1) ? "hover:text-blue-300" : "hover:text-blue-700"
    }
      ...
      >
        ...
        <div className="hidden md:flex items-center space-x-8">
          {navItems.map((item) => (
            ...
            >
              {item.name}
              <span className={`absolute bottom-0 left-0 w-0 h-0.5 bg-blue-500 transition-all duration-300 ease-out group-hover:w-full ${
          ...
          <a
            ...
            className={`p-2 rounded-full transition-all duration-300 hover:scale-110 ${
              (pathname === "/" && currentSlide <= 1)
                ? "text-white hover:bg-green-500/20 drop-shadow-lg"
        ...
        {/* Mobile Menu Button */}
        <button
          className={`md:hidden transition-colors ${getTextColor()} ${getHoverColor()}`}
          ...
          <div className="flex flex-col items-center justify-center h-full space-y-8">
            {navItems.map((item) => (
              <button
                ...
                } ${isActivePage(item.href) ? ((pathname === "/" && currentSlide <= 1) ? "text-blue-300" : "text-blue-700") : ""}`}
              >
                {item.name}
              </button>
            ))}
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`text-2xl font-bold tracking-wider transition-colors duration-300 flex items-center space-x-2 ${
                (pathname === "/" && currentSlide <= 1)
